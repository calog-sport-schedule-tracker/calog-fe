<template>
  ...
  export default {
    ...
    methods: {
      // 전체 이벤트 가져오기
      fetchEvents() {
        const uStore = useUserStore(); // 유저 스토어 사용
        const userId = uStore.userId;
        console.log("Fetch Events 호출 - User ID:", userId); // 로그 추가
        axios
          .get(`http://localhost:8080/api/favorite/user/1`)
          .then((response) => {
            console.log("가져온 이벤트 데이터:", response.data); // 가져온 데이터 확인
      ...
      updateCalendarEvents(events) {
        console.log("Calendar 업데이트 이벤트:", events); // 가져온 원본 이벤트 로그
        ...
        console.log("Formatted Events:", formattedEvents); // 변환된 이벤트 로그
    ...
    mounted() {
      ...
      this.fetchEvents();
      console.log("초기 Calendar Options:", this.calendarOptions); // 초기 옵션 로그
    },
  };
  </script>
